...
See [PLAN.md](./PLAN.md) for the architecture and phased build plan.
...
The apps themselves (About, Projects, Resume, Contact, Terminal, Trash) are
...
| `Esc` | Closes the focused window. Inside a text field, the first press just leaves the field. |
| `↑ ↓ ← →` | Moves the focused window by 24px, when the frame itself has focus. |
