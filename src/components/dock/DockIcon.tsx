...
export function DockIcon({ app }: { app: AppDefinition }) {
  ...
  const isRunning = useDesktop((state) =>
    state.windows.some((win) => win.appId === app.id),
  );
    ...
    if (enabled) {
      ...
      if (isRunning) {
        ...
        bounce.start({
          ...
          transition: {
            duration: 0.85,
            times: [0, 0.16, 0.34, 0.48, 0.64, 0.82, 1],
            ease: "easeOut",
          },
        });
      }
  ...
  return (
    ...
    >
      ...
      <motion.button
        ...
        onClick={handleClick}
        animate={bounce}
        className="flex shrink-0 items-center justify-center rounded-[24%] border border-white/20 text-white shadow-[inset_0_1px_0_oklch(1_0_0/0.3),0_8px_18px_-8px_oklch(0_0_0/0.85)]"
        style={{
